...
var PD = (function () {
  ...
  function deleteProduct(id) {
    ...
    if (!window.PDSupabase || !window.PDSupabase.deleteProduct) {
      return Promise.reject(new Error('Supabase indisponivel para excluir o produto.'));
    }
    return window.PDSupabase.deleteProduct(id)
      .then(function() {
        if (window.PDSupabase.clearProductsCache) window.PDSupabase.clearProductsCache();
        setOnlineProducts(products, 'supabase');
        refreshDynamicViews();
        return products;
      });
